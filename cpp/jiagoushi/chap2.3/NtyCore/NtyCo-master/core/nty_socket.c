...
static int nty_poll_inner(struct pollfd *fds, nfds_t nfds, int timeout) {
	...
	int i = 0;
	for (i = 0;i < nfds;i ++) { // 将其加入到主线程中去，然后等待
	
		struct epoll_event ev;
		ev.events = nty_pollevent_2epoll(fds[i].events);
		ev.data.fd = fds[i].fd;  // 将poller的事件转换为epoll的事件
	...
	nty_coroutine_yield(co);   // 让出cpu使用权，利用主epoll去检测
	//当有IO数据时，得到调度又回来了
	...
	for (i = 0;i < nfds;i ++) {
		...
		struct epoll_event ev;
		ev.events = nty_pollevent_2epoll(fds[i].events);
		ev.data.fd = fds[i].fd;    // 当处理完数据后，在将其在epoll中
		//删除,就告诉主线程你不要帮我监听了
...
int nty_accept(int fd, struct sockaddr *addr, socklen_t *len) {
	...
	while (1) {
		...
		nty_poll_inner(&fds, 1, timeout); // 将监听套接字进行封装
