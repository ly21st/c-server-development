...
int ikcp_recv(ikcpcb *kcp, char *buffer, int len)
{
    ...
    // 将合适的数据从接收缓存 转移到接收队列, 已经确认的以及接收窗口未满
	while (! iqueue_is_empty(&kcp->rcv_buf)) {
		seg = iqueue_entry(kcp->rcv_buf.next, IKCPSEG, node);
        // 条件1 序号是该接收的数据
...
//---------------------------------------------------------------------
// parse data 将该报文插入到 rcv_buf 链表中
// 1. 先判断包是否需要处理， 不在范围内的、重复的就不需要处理了
// 2.
//---------------------------------------------------------------------
void ikcp_parse_data(ikcpcb *kcp, IKCPSEG *newseg)
{
	...
	if (repeat == 0) {
		...
		iqueue_add(&newseg->node, p);   // 加入到队列
		kcp->nrcv_buf++;
	}	else {
...
void ikcp_flush(ikcpcb *kcp)
{
	...
	seg.una = kcp->rcv_nxt;    // 已经处理到哪一个包了
	seg.len = 0;
	seg.sn = 0;
	seg.ts = 0;
	...
	if (kcp->probe & IKCP_ASK_TELL) {
		seg.cmd = IKCP_CMD_WINS;    // [告诉我方窗口size], 此时不为0，可以往我方发送数据
		size = (int)(ptr - buffer);
		if (size + (int)IKCP_OVERHEAD > (int)kcp->mtu) {
			ikcp_output(kcp, buffer, size);
			ptr = buffer;
